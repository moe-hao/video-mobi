...
export async function getDisputeOrderInfo(req: DisputeOrderReq): Promise<DisputeOrderResp> {
    const orderInfo = await orderDao.getDisputeOrderInfo(req.search);

    const [userList, orderSkuList] = await Promise.all([
        userDao.getUserListByIds([orderInfo.userId]),
        skuDao.getSkuListByIdsNoStatus([orderInfo.skuId]),
    ]);

    const [userInfo] = userList;
    const [orderSkuInfo] = orderSkuList;
    ...
    return {
        ...
        userNo: userInfo?.bizId || '--',
        amount: orderInfo.amount,
        currency: orderInfo.currency,
        orderType: orderInfo.orderType as SkuType,
        subscriptionId: orderInfo.subscriptionId,
        subscriptionCount: orderInfo.subscriptionCount,
        subscriptionPeriod: SkuPeriodTypeName[orderSkuInfo?.periodType as SkuPeriodType] || '',
        paymentChennel: orderInfo.paymentChannel,
        paymentType: orderInfo.paymentType as PaymentType,
        paymentTypeName: PaymentTypeName[orderInfo.paymentType as PaymentType],
        orderStatus: orderInfo.orderStatus as OrderStatus,
        orderStatusName: OrderStatusName[orderInfo.orderStatus as OrderStatus],
        createTime: formatUnixTime(orderInfo.createTime),
        updateTime: formatUnixTime(orderInfo.updateTime),
    }
}
