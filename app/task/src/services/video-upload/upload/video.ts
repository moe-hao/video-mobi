...
export async function getVideoAuth(): Promise<string> {
    const resp = await fetch('https://br4.beeshortframe.com/shorts/user/loginByDevice', {
        method: 'POST',
        headers: {
...
export async function getVideoList(videoAuth: string, videoId: number, episodes: number): Promise<{ num: number, playUrl: string, coverImage: string }[]> {
    const resp = await fetch(`https://br4.beeshortframe.com/shorts/volc/playUrlList/${videoId}/0/${episodes}`, {
        method: 'GET',
        headers: {
