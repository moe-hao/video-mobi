...
export class SubscriptionDao {
    ...
    async getSubscriptionPageList(page: number, size: number): Promise<SubscriptionSelect[]> {
        return await this.conn.select().from(subscriptionTable).limit(size).offset((page - 1) * size).orderBy(desc(subscriptionTable.id));
    }

    async getSubscriptionPageTotal(): Promise<number> {
