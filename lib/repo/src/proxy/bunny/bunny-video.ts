...
export class BunnyVideoProxy {
    constructor(
        private readonly videoBaseURL: string = 'https://video.bunnycdn.com/library',
    ...
    async getVideoInfo(bid: string): Promise<GetVideoInfoResult> {
        const videoInfoURL = `${this.videoBaseURL}/${this.libraryId}/videos/${bid}`;
        const resp = await fetch(videoInfoURL, {
            method: 'GET',
            headers: {
                'AccessKey': this.accessKey,
                'Content-Type': 'application/json'
        ...
        const result = await resp.json() as GetVideoInfoResult;
        return result;
    }
}

export const bunnyVideoProxy = new BunnyVideoProxy();
